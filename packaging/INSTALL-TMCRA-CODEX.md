...
Restart Codex Desktop, confirm `TMCRA Memory` is enabled in Plugins, run `/hooks`, and explicitly review all nine lifecycle hooks. Complete one turn in a new task, then verify real Codex execution in the TMCRA Memory app. Ready requires observed SessionStart, recall, and capture events from the current plugin version.
